...
public class TicketForm extends JFrame{
	...
	public TicketForm(String selectmemberNum, String CuisineNum, String alltime, DefaultTableModel model) {
		...
		JPanel panel2 = new JPanel(); // 나열된 식권들을 받아올 panel, scrollpane에 올릴 panel
		...
		jsp.setVerticalScrollBarPolicy(JScrollPane.VERTICAL_SCROLLBAR_AS_NEEDED); // 스크롤이 생기는 기준
		// 1개만 선택헀을 때, 2개이상일때
		if(cnt<2) {jsp.setBounds(0, 0, 310, 200);}
		else {jsp.setBounds(0, 0, 310, 365);}
		this.add(jsp);
		
		// 1개만 선택헀을 때, 2개이상일때
		if(cnt<2) {setSize(325,230);}
	...
	class TicketAdd extends JPanel{
		public TicketAdd() { 
			...
			this.setBorder(blackline); // Border 설정
	...
	class CenterP extends JPanel{
		public CenterP() {
			...
			JLabel label1 = new JLabel("식권",SwingConstants.CENTER); // 식권 타이틀, 중간에 배치
			...
			JLabel label2 = new JLabel(format_price + "원", SwingConstants.CENTER); // 가격, 중간에 배치
	...
	class SouthP extends JPanel{
		public SouthP() {
			...
			String menuname = (String) model.getValueAt(currow, 1); // 메뉴명
			int allCount = Integer.parseInt((String) model.getValueAt(currow, 2));
			JLabel label1 = new JLabel("메뉴:" + menuname, SwingConstants.LEFT); // 메뉴명 라벨, 왼쪽에 배치
			JLabel label2 = new JLabel( curnum + "/" + allCount, SwingConstants.RIGHT); // 현재번호/전체수량, 오른쪽에 배치
	...
	// 사진 저장
	public void SaveImg() {
		JComponent view = (JComponent) jsp.getViewport().getView(); // 스크롤팬 안에 보이는 부분
		
		BufferedImage img = new BufferedImage(view.getWidth(),view.getHeight(),BufferedImage.TYPE_INT_RGB);
		Graphics2D g2 = img.createGraphics(); 
		view.paintAll(g2); // 중요, 스크롤로 인해 안보이는 부분까지 다 그리기
		try {
			ImageIO.write(img, "jpg", new File("imgs/"+ oneline + "-ticket.jpg")); // 이미지 저장 경로는 imgs폴더 안에
		}catch (Exception e) {
